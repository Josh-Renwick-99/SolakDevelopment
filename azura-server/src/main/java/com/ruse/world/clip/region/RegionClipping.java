...
public final class RegionClipping {
	...
	public static void init() {
		try {
			...
			final File file = new File("./azura-server/data/clipping/map_index");
			if (!file.exists()) {
				return;
			}
	...
	public static boolean objectExists(GameObject object) {
		...
		boolean dgStart = id == 10014 && pos.getX() == 2250 && pos.getY() == 5043;
		...
		if (well || mageBankLever || lawAltar || trees || chaosTunnels || lunar || barrows || rfd || lumbridgeCastle
				...
				|| barrows || waterRcAltar || crystalChest || draynor || dgStart)
			return true;
