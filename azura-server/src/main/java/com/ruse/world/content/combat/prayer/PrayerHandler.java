...
public class PrayerHandler {
	...
	private enum PrayerData {
		...
		FORTITUDE(85, 11, -23029, 692),
		GNOMES_GREED(90, 12, -23027, 693),
		SOUL_LEECH(94, 12, -23025, 694, 9),
		FURY_SWIPE(99, 13, -23023, 695);
            ...
            if (buttonId == pd.buttonId) {
                if (!player.getPrayerActive()[pd.ordinal()]) {
                    ...
                    if(player.getMiniPManager().getMiniPlayer() != null)
                        activatePrayer(player.getMiniPManager().getMiniPlayer(), pd.ordinal());
                } else {
                    deactivatePrayer(player, pd.ordinal());
                    if(player.getMiniPManager().getMiniPlayer() != null)
                        deactivatePrayer(player.getMiniPManager().getMiniPlayer(), pd.ordinal());
                }
            }
        }
    }
	...
	public static void activatePrayer(Player player, final int prayerId) {
		...
		if(prayerId == SOUL_LEECH && !player.isHolyPrayerUnlocked(HOLY_SOUL_LEECH_IDX)) {
			player.getPacketSender().sendConfig(pd.configId, 0);
			player.getPacketSender().sendMessage("You need to unlock Soul leech with a holy prayer scroll before using it!");
			return;
		}
		...
		System.out.println("activate prayer");
		if (hasNoPrayerOn(player, prayerId) && !player.isDrainingPrayer()) {
			System.out.println("startDrain");
			startDrain(player);
		}
		BonusManager.sendCurseBonuses(player);
		if (pd.hint != -1) {
	...
	public static void startDrain(final Player player) {
		if (getDrain(player) <= 0 && !player.isDrainingPrayer() || player instanceof MiniPlayer)
			return;
		...
		TaskManager.submit(new Task(1, player, true) {
			...
			public void execute() {
				...
				/*if(player.switchedPrayerBooks) {
					System.out.println("switchedPrayerBooks");
