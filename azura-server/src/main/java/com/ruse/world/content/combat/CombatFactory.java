...
import com.ruse.world.content.raids.totg_raids.TrialOfGods;
...
public final class CombatFactory {
        ...
        if (!(entity.isNpc() && ((NPC) entity).isSummoningNpc())) {
            ...
            if (victim.getCombatBuilder().getLastAttacker() != null && !Location.inMulti(entity)
                ...
                if (!allowAttack && !isMultiNPC) {
                    if (entity.isPlayer()) {
                        if (!(entity instanceof MiniPlayer) && (((Player) entity).getMiniPManager().getMiniPlayer() == null)) {
    ...
    protected static void applyPrayerProtection(CombatContainer container, CombatBuilder builder) {
        ...
        if (builder.getVictim().isPlayer()) {
            ...
            } else if(npc.getId() == TrialOfGods.VINDICTA_NPC_ID && (container.getCombatType() == RANGED || container.getCombatType() == MAGIC)) {
                // Absorb 90% of ranged/maged
                container.allHits(context -> {
                    int hit = context.getHit().getDamage();
                    double mod = Math.abs(1 - 0.1);
                    context.getHit().incrementAbsorbedDamage((int) (hit - (hit * mod)));
    ...
    protected static void giveExperience(CombatBuilder builder, CombatContainer container, int damage) {
        ...
        if (builder.getCharacter().isPlayer()) {
            ...
            if (container.getCombatType() == CombatType.MAGIC) {
                ...
                if (player instanceof MiniPlayer && container.getCombatType() == CombatType.MELEE) {
					player.getSkillManager().addExperience(Skill.ATTACK,
							(int) (((damage * .90)) / container.getExperience().length));
					player.getSkillManager().addExperience(Skill.STRENGTH,
							(int) (((damage * .90)) / container.getExperience().length));
					player.getSkillManager().addExperience(Skill.DEFENCE,
							(int) (((damage * .90)) / container.getExperience().length));
				} else {
    ...
    protected static void handlePrayerEffects(Character attacker, Character target, int damage, CombatType combatType) {
        ...
        if (attacker.isPlayer()) {
            ...
            if (PrayerHandler.isActivated(p, PrayerHandler.SOUL_LEECH) && damage > 0) {
                ...
                if (Misc.getRandom(100) <= 5) {
                    p.setDoubleDMGTimer(1);
                    //p.getPacketSender().sendMessage("Soul leech activated");
                }
